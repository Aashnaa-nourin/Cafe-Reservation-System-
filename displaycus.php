...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    $NORMAL_SQL = "SELECT * FROM `register`";
    ...
    function FancyTable($header, $staff)
    {
        ...
        foreach ($staff as $result) {
            ...
            $this->SetXY($x0, $y);
            $this->MultiCell($w[0], 7.5, $result['name'], 'LRT', 'L', $fill);
            $this->SetXY($x, $y);
...
// Instanciation of inherited class
    ...
    $pdf->Cell(190, 10, ' Customer Report | Cafearo ', 0, 0, 'C');
    // Line break
    $pdf->Ln(20);

    $titles = array("Name", "Email", "Number");
    $pdf->FancyTable($titles, $staffDetails);

    $pdf->Output('D', 'Customer Details.pdf');
}
?>
...
<form action="" method="post">
    ...
    <div id="hero">
        <div style="display: flex;justify-content: start;padding: 130px; ">
            <div style="width: 100%;">
                <h2 id="hero_head">MANAGE CUSTOMER DETAILS</h2><br><br>
                ...
                <p id="hero_content"> 
                    <table>
                        ...
                        <?php
                        $sel=$conn->query("SELECT * FROM `register` inner join `login` on `register`.email = `login`.username where `login`.status='customer'");
                        if($sel->num_rows>0){
                            while($result=mysqli_fetch_array($sel)){
                                ...
                                <form method="POST">
                                    <?php  if($status == 1){
                                    ?>
                                    <a  class='bt'  href='./status/custStatus.php?email=<?php echo $email; ?>&status=0'>Deactivate</a><br></td>
                                    <?php                                         
                                    }else{
                                        ?>
                                    <a style="background-color: green;"class='bt activet' href='./status/custStatus.php?email=<?php echo $email; ?>&status=1''>Activate</a><br></td>
                                        <?php
                                    }
                                        ?>
