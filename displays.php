...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    $NORMAL_SQL = "SELECT u_name,email,`address`,phn FROM `user` where type='staff'";
...
// Instanciation of inherited class
    ...
    $pdf->Cell(190, 10, ' Staff Report | Cafearo ', 0, 0, 'C');
    // Line break
    $pdf->Ln(20);
    ...
    $pdf->Output('D', 'staff Details.pdf');
}
...
<form action="" method="post">
    ...
    <div id="hero">
        <div style="display: flex;justify-content: start;padding: 130px; ">
            <div style="width: 100%;">
                <h2 id="hero_head">MANAGE STAFF DETAILS</h2><br><br>
                ...
                <p id="hero_content"> 
                    <table>
                        ...
                        <?php
                    include("./conn.php");
                    $sel=$conn->query("SELECT * FROM `user` inner join `login` on `user`.email=`login`.username where `login`.status='staff'");
                    if($sel->num_rows>0){
                            while($result=mysqli_fetch_array($sel)){
                                ...
                                <form method="POST">
                                    <?php  if($status == 1){
                                    ?>
                                    <a  class='bt'  href='./status/staffStatus.php?email=<?php echo $email; ?>&status=0'>Deactivate</a><br></td>
                                    <?php                                         
                                    }else{
                                        ?>
                                    <a style="background-color: green;"class='bt activet' href='./status/staffStatus.php?email=<?php echo $email; ?>&status=1''>Activate</a><br></td>
                                        <?php
                                    }
                                        ?>
                                <td>
                        ...
                        if(isset($_POST['delete'])){
                            ...
                            if(!empty($a)){
                                ...
                                echo"deleted";
                                echo "<script>window.location.replace('../cafe/displays.php')</script>";
                            }else{
                       ...
                       <script>
                        const fn = (id) => {
                            console.log(id);
                            window.location.replace(`./editstaff.php?id=${id}`);
                        }
                        </script>
<?php 
//     if($_SERVER['REQUEST_METHOD'] == "POST"){
...
// $NORMAL_SQL = "SELECT u_name,email,`address`,phn,exp FROM `user` where type='staff'";

// $stmt = $conn->query($NORMAL_SQL);
// $satffDetails = $stmt->fetch_assoc();
...
// $pdf->SetFillColor(230,230,0);
// $pdf->SetLineWidth(0.4);
// $pdf->Cell(190,10,' Satff Report | Cafearo ',0,0,'C');
// // Line break
// $pdf->Ln(20);

// foreach ($carts as $i => $cart) {
//     // $pdf->SetFont('Arial','',12);
//     // $pdf->Cell(100,10,"Report Number : {$cart['order_id']}");
//     // $pdf->Ln();
