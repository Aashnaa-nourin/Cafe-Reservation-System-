...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    $NORMAL_SQL = "SELECT u_name,email,`address`,phn FROM `user` where type='waiter'";
...
// Instanciation of inherited class
    ...
    $pdf->Cell(190, 10, ' Waiter Report | Cafearo ', 0, 0, 'C');
    // Line break
    $pdf->Ln(20);
    ...
    $pdf->Output('D', 'Waiter Details.pdf');
}
...
<form action="" method="post">
    ...
    <div id="hero">
        <div style="display: flex;justify-content: start;padding: 130px; ">
            <div style="width: 100%;">
                <h2 id="hero_head">MANAGE WAITER DETAILS</h2><br><br>
                <!-- <a href="adminwaiter.html" class="bt1">back</a> -->
                <p id="hero_content"> 
                    <table>
                        <tr>
                            <th>ID</th>
                        ...
                        $sel=$con->query("SELECT * FROM `user`  inner join `login`  on `user`.email=`login`.username where `login`.status='waiter'");
                        if($sel->num_rows>0){
                            while($result=mysqli_fetch_array($sel)){
                                ...
                                <form method="POST">
                                    <?php  if($status == 1){
                                    ?>
                                    <a  class='bt'  href='./status/waiterStatus.php?email=<?php echo $email; ?>&status=0'>Deactivate</a><br></td>
                                    <?php                                         
                                    }else{
                                        ?>
                                    <a style="background-color: green;"class='bt activet' href='./status/waiterStatus.php?email=<?php echo $email; ?>&status=1''>Activate</a><br></td>
                                        <?php
                                    }
                                        ?>
                                <td>
