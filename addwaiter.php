...
if (isset($_POST['add'])) {
    ...
    if (empty(array_filter($errors))) {
        ...
        if (isEmailExists($conn, $b)) {
            ...
            $inss=$conn->query("INSERT INTO login(username, password, status) VALUES ('$b','$g','waiter')");
            $ins=$conn->query("INSERT INTO `user`( `u_name`, `email`, `phn`, `address`, `doj`, `exp`, `password`,`type`) VALUES ('$a','$b','$c','$d','$e','$f','$g','waiter')");
            // echo "$inss";
            // echo "$inss";
            if ($inss) {
...
<html>
<head>
    <title>Add Waiter Details</title>
    <style>
        h1 {
            text-align: center;
        }
...
<body>
    ...
    <form method="post" action="">
        ...
        <input type="submit" name="add" value="Add waiter">
    </form>
</body>
</html>